...
具体登录方式采用百度开放平台 OAuth 授权码流程：

1. 用户在本地页面点击“绑定百度账号”。
2. 本地服务跳转到百度授权页。
3. 用户在百度页面完成扫码、登录和授权。
4. 百度把浏览器重定向回本地 `redirect_uri`。
5. 本地服务拿到 `code`，再用应用的 `client_id` 和 `client_secret` 去换 `access_token` 和 `refresh_token`。
6. 本地只加密保存 `refresh_token` 和必要的少量会话信息。
7. 后续由本地自动刷新 `access_token`。

这个流程不依赖第三方 token 中转服务。用户侧不需要额外的开发者权限，但项目本身需要百度开放平台应用信息。
